...
if __name__ == "__main__":
    ...
    if visualize:
        ...
        viz.add_marker(pancakePosition_in_w,yellow)
        viz.add_marker(platePosition_in_w,green)
        viz.add_frame(np.eye(4),'World')
        viz.add_obstacle(tablePosition_in_w,color=grey,rad=tableInfluenceRadius)
    ...
    approachDistance = pancakeDiameter + 0.1 # [meters]
    heightClearance = 0.3 # [meters]
    i = 4
    preScoopPosition_world = pancakePosition_in_w - approachDirections_world[i] * approachDistance + np.array([0,0,heightClearance])
    R = tr.rotz(np.atan2(approachDirections_world[i][1], approachDirections_world[i][0]))
    T_preScoop_world = tr.se3(R=R, p=preScoopPosition_world)

    if visualize:
        viz.add_frame(T_preScoop_world, 'preScoop')
        input('press Enter to see next iteration')



    # Step 1: move to pancake
    T_tip_target_world = T_preScoop_world
    ...
    # Step 2: Flip Pancake
    q1 = q_new
    T_byPancake = T_tip_target_world
